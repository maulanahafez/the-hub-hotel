<x-layout.layout>
    <section class="mx-auto mb-2 mt-[4rem] max-w-2xl p-4">
        <div class="container relative mx-auto">
            <div class="group relative max-h-80 overflow-hidden rounded-lg" x-data="{ selected: 1, count: 4 }">
                <img src="{{ asset('images/hotel-2.jpg') }}" alt="" class="w-full object-cover"
                    x-show="selected === 1" x-transition:enter="transition-all duration-500 ease-in-out"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" />
                <img src="{{ asset('images/hotel-2.jpg') }}" alt="" class="w-full object-cover"
                    x-show="selected === 2" x-transition:enter="transition-all duration-500 ease-in-out"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" />
                <img src="{{ asset('images/hotel-2.jpg') }}" alt="" class="w-full object-cover"
                    x-show="selected === 3" x-transition:enter="transition-all duration-500 ease-in-out"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" />
                <img src="{{ asset('images/hotel-2.jpg') }}" alt="" class="w-full object-cover"
                    x-show="selected === 4" x-transition:enter="transition-all duration-500 ease-in-out"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" />
                <span
            ...
            <form method="post" action="{{ route('home.reservation', ['roomType' => $roomType->slug]) }}"
                class="mt-6">
                @csrf
                <button type="submit"
            ...
            <div class="mt-6">
                ...
                <div class="mt-8">
                    <h1 class="font-poppins text-sky-950 relative text-2xl font-semibold">
                        ...
                        <span
                            class="bg-sky-950 absolute right-2 top-1/2 hidden h-px w-80 -translate-y-1/2 rounded-sm sm:inline">
                        </span>
                    </h1>
